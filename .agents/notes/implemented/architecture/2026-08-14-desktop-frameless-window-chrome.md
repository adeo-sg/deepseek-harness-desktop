...
- **The client plugin** (`packages/client/ui-window-controls`, `@deepseek-ai/dsh-client-ui-window-controls`): a pure-presentation cluster (three buttons, inline-SVG glyphs, tokens-only CSS, Chinese aria labels) coordinated by two registrations, both through `slots.inject` on their declaring entries:
