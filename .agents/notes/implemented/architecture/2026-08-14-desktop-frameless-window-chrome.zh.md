...
- **客户端插件**（`packages/client/ui-window-controls`，`@deepseek-ai/dsh-client-ui-window-controls`）：纯展示按钮簇（三个按钮、内联 SVG 字形、仅用 token 的 CSS、中文 aria 文案），注册两处，都经由 `slots.inject` 跟随声明方生命周期：
  - `conversation.session.header.utilities`（order 100，位于 Session-log 工具右侧）——内联占位者，位于会话页头右端，即窗口右上角；
  - `shell.overlay`——浮动占位者，钉在窗口右上角，**仅在会话页头隐藏时**渲染（没有当前会话，或空白会话处于新建会话 hero），因此第一条消息存在之前窗口依然可以关闭。任一时刻只有一个实例渲染；该行只存在于 `dsh-desktop-app` bundle 补丁中（外加 bundle 与应用自身的依赖闭包），web 组合永远不会加载该插件，组件在 preload 表面缺失时（fixture 模式、意外名单）也渲染为空。
