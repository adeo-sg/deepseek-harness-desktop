...
桌面壳层是 `apps/desktop`（`@deepseek-ai/dsh-desktop`，Electron）：它通过 CLI 所用的同一套 `dsh-app-boot` 机制在进程内启动 `desktop` profile（`@deepseek-ai/dsh-base` + `@deepseek-ai/dsh-web-app` + `@deepseek-ai/dsh-desktop-app` 三个 bundle），Harness home 可临时化，模块回退经 heal 建立。三块实现载体替换：
