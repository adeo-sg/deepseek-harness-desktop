...
Updater tests cover state transitions, scheduling, live settings, duplicate-action guards, manual-only builds, provider and installer failures, IPC ordering, tray failure containment, and renderer presentation. A provider-level test parses a representative GitHub Atom feed with `dsh-v` release-candidate tags and verifies that discovery retains the original asset URLs. The packaged-boot smoke loads the shipped preload bridge and client bundle. The snapshot harness has no Electron preload lane, so the desktop-only banner has no assembled browser snapshot.
