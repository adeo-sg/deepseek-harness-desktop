...
The [dsh artifact workflow](../../../../.github/workflows/container-release.yml) builds amd64 and arm64 images on native runners, saves each image, removes its local tag, reloads the saved bytes, and health-checks the restored image. Before upload, it verifies and extracts the deployment archive and validates its internal manifest. The amd64 runner also starts the extracted Compose definition with its hardened filesystem and verifies HTTP health plus `/data` and `/workspace` persistence across container replacement, as decided in [Load the container native Loader helper from its installed path](../bug-fix/2026-08-15-noexec-container-native-loader.md). A `dsh-v<version>` tag publishes both image archives, their SHA-256 files, and the deployment archive with its checksum as part of the [unified GitHub Release](2026-08-15-unified-native-release-assets.md); manual runs retain the same files as 30-day Actions artifacts. The workflow has no image-registry credentials or writes. The [deployment packer](../../../../scripts/release/pack-container.ts) copies Compose, Kubernetes, and guide assets, removes the source-only `build` section from the packaged Compose file, pins Compose and Kubernetes to the saved image's local name and version, records per-file hashes, and emits a SHA-256 checksum for the archive.
