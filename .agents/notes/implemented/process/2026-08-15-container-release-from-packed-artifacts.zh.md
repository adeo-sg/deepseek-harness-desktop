...
[dsh 产物工作流](../../../../.github/workflows/container-release.yml)会在原生 runner 上构建 amd64 和 arm64 镜像，保存每个镜像，删除其本地 tag，重新加载已保存的字节，并对恢复后的镜像执行健康检查。上传前，工作流会校验并解压部署归档，再验证其内部 manifest。amd64 runner 还会使用加固文件系统启动解压后的 Compose 定义，并验证 HTTP 健康以及容器替换前后的 `/data` 与 `/workspace` 持久化，具体遵循[从安装路径加载容器原生 Loader 辅助组件](../bug-fix/2026-08-15-noexec-container-native-loader.md)的决策。`dsh-v<version>` tag 会将两个镜像归档、对应 SHA-256 文件、部署归档及其 checksum 作为[统一 GitHub Release](2026-08-15-unified-native-release-assets.md)的一部分发布；手动运行会将相同文件保留为 30 天的 Actions artifact。工作流不持有镜像 registry 凭据，也不写入 registry。[部署打包器](../../../../scripts/release/pack-container.ts)会复制 Compose、Kubernetes 和指南资产，从打包后的 Compose 文件中移除只用于源码的 `build` 部分，将 Compose 和 Kubernetes 固定到保存镜像的本地名称与版本，记录逐文件哈希，并为归档生成 SHA-256 checksum。
