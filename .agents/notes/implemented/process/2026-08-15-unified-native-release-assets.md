...
The assembled Release contains exactly 29 assets: 16 Desktop installers and portable archives, two macOS blockmaps, four update metadata files, the deployment archive and checksum, two image archives and their checksums, and one `SHA256SUMS` covering the other 28 files. Uploads remain in a draft until the workflow verifies exact asset names, every remote size, and downloaded copies of `SHA256SUMS` and updater metadata. It then publishes the verified draft. A retry replaces only an incomplete draft; an existing public Release remains read-only and must already match the verified collection.
