...
The assembled Release contains exactly 29 assets: 16 Desktop installers and portable archives, two macOS blockmaps, four update metadata files, the deployment archive and checksum, two image archives and their checksums, and one `SHA256SUMS` covering the other 28 files. After upload, the workflow compares the published GitHub asset names and count with this local set.
