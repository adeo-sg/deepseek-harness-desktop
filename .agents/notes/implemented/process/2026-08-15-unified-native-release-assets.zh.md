...
汇总后的 Release 恰好包含 29 个资产：16 个 Desktop 安装包和便携归档、两个 macOS blockmap、四个更新元数据文件、部署归档及其 checksum、两个镜像归档及其 checksum，以及一个覆盖其余 28 个文件的 `SHA256SUMS`。上传后，工作流会将 GitHub 上已发布资产的名称和数量与该本地集合比较。
