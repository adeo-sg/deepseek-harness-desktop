...
生产 Web 运行器需要已构建的包和前端产物（`pnpm run build`）。默认服务地址是 `http://127.0.0.1:3080`，这也是 npx/本地开发端口。对外绑定需使用 `--host 0.0.0.0 --allow-non-loopback`，并放在具备认证且终止 TLS 的网络边界之后；`--trusted-host` 可添加 `/api` 浏览器信任围栏接受的外部 authority。
