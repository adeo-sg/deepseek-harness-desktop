/**
 * ui-updater plugin halves: the browser entry registers the update banner and
...
describe('ui-updater browser half', () => {
    ...
    try {
      ...
      await vi.waitFor(() => {
        expect(error).toHaveBeenCalledWith(
          'dsh ui-updater: initial state unavailable:',
...
describe('ui-updater invariant companion', () => {
  it('reserves package ownership under its declared companion name', async () => {
    const ctx = new Context()
    await ctx.plugin(InvariantRegistry, { enabled: true })
    const fiber = ctx.plugin(UpdaterInvariant)
    await fiber.await()
    expect(UpdaterInvariant.name).toBe('client-ui-updater-invariant')
